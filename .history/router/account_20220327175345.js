...
router.get('/getFundAccount', (req, res) => {
  handler.exec({
    sql: 'SELECT userinout.inoutType,useraccount.accountBalance,useraccount.accountType,userinout.accountName,SUM(userinout.balance) '+
    'FROM userinout LEFT JOIN useraccount' +
    'on userinout.id = useraccount.id AND useraccount.accountName=userinout.accountName'+
    'WHERE useraccount.id=?'+
    'GROUP BY userinout.accountName,userinout.inoutType',
    params: [req.query.id],
    success: (result) => {
      res.send({ data: result });
      console.log('dfsdf', result,sql);
    },
